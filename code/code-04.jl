...
Draws a general Archimedan spiral through n turns with the given offset and radius:
...
using Plots
function ArchimedanSpiral(a, b, n, c=1)
    m = 10000;  Δangle = 2*pi*n/m;
    θ = [(i-1)*Δangle for i in 1:m]
    r = a .+ (b .* θ)
    x = zeros(m); y = zeros(m);
    for i in 1:m
        x[i] = r[i]cos(θ[i])
    ...
    label = "r = $a + $b θ"
   plot(x, y, title = "Archimedian Spiral",
   xlabel = "x = rcos(theta)", ylabel = "y = rsin(theta)",
   label = "r = $a + $b"*"*theta")
end
